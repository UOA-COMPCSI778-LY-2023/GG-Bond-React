...
const shipIcon = (heading, type) => {
    ...
    return L.divIcon({
      ...
      html: ReactDOMServer.renderToString(
        <FiNavigation2 className='ships' style={{strokeColor: "black", fill: color, transform: `rotate(${heading}deg) scale(1.5)` }} />
      ),
    });
};
...
const ShipMarker = ({ boatData, setSelectedBoat, isSelected }) => {
  ...
  return (
    <>
        ...
        {isSelected && (
        <Marker position={[location.latitude, location.longitude]} icon={targetIcon()}>
        </Marker>
      )}
